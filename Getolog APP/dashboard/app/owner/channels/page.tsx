...
export default function OwnerChannelsPage() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-lg font-medium text-h-ink">Kanal va guruhlar</h1>
      ...
      {loading ? (
        ...
        <p className="text-sm text-h-muted">Hali birorta ham kanal yoki guruh ulanmagan.</p>
      ) : (
        <>
          ...
          <div className="overflow-x-auto rounded-lg border border-h-border bg-h-surface">
            <table className="min-w-full divide-y divide-h-border text-sm">
              ...
              <tbody className="divide-y divide-h-border">
                {channels.map((c) => (
                  <tr key={c.id}>
                    <td className="px-4 py-3 font-medium text-h-ink">
                      {c.title}
                      <span className="ml-1.5 text-xs font-normal text-h-muted">
                        ({c.chat_type === "group" ? "guruh" : "kanal"})
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <a
                        href={`https://t.me/${c.bot_username}`}
                        target="_blank"
                        rel="noreferrer"
                    ...
                    <td className="px-4 py-3 text-h-muted">
                      ...
                      {c.admin_username && (
                        <a
                          href={`https://t.me/${c.admin_username}`}
                          target="_blank"
                          rel="noreferrer"
                          className="ml-1.5 text-h-accent hover:underline"
                        >
                          @{c.admin_username}
                        </a>
                      )}
